 ...
 * Each sRGB primary is modelled as a Gaussian emission lobe sitting on a broad
 * reflectance pedestal that reaches into the near-IR and near-UV — so a strongly
 * blueshifted object reveals the infrared it was radiating all along, and a
 * redshifted one reveals its ultraviolet, before both fade to black.
 *
...
(function (R) {
  ...
  function sourceSpectrum(ch, lambda) {
    if (lambda < 120 || lambda > 6000) return 0;
    const p = LOBE[ch];
  ...
  function spectrumStops(steps, dMin, dMax) {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      let r = (N[0] + N[1] + N[2]) * 0.8;
      let g = (N[3] + N[4] + N[5]) * 0.8;
      let b = (N[6] + N[7] + N[8]) * 0.8;
      // Renormalise brightness so the bar reads as hue, not exposure.
      const m = Math.max(r, g, b, 1e-4);
      const k = 1 / Math.pow(m, 0.82);
      const enc = v => Math.round(255 * Math.pow(Math.min(1, Math.max(0, v * k)), 1 / 2.2));
      out.push({ f, css: 'rgb(' + enc(r) + ',' + enc(g) + ',' + enc(b) + ')' });
    }
    return out;
  }
